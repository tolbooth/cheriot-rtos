...
#	endif
	...
	void test_claims()
	{
		...
		CHERI::Capability badAlloc{
		  heap_allocate(&noWait, MALLOC_CAPABILITY, 0)};
		TEST(!badAlloc.is_valid(),
		     "Expected invalid allocation, returned {}",
		     badAlloc);
		claimSize = heap_claim(SECOND_HEAP, badAlloc);
