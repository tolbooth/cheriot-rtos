...
namespace
{
	...
	template<typename T>
	concept IdAllocationPolicy = requires(uint32_t gap) {
		{ T::pick_offset(gap) } -> std::same_as<uint16_t>;
	};

	struct MidpointIdPolicy
	{
		static uint16_t pick_offset(uint32_t gap)
		{
			Debug::Assert(gap > 0, "pick_offset called with empty gap");
			return static_cast<uint16_t>(gap / 2);
		}
	};

	template<IdAllocationPolicy Policy = MidpointIdPolicy>
	class SubQuotaIdSpace
	{
		static constexpr uint16_t IdMax =
		  (1 << (MChunkHeader::OwnerIDWidth)) - 1;
		// TODO: If we can determine the number of static quotas in the
		...
		void
		insert_after(PrivateAllocatorCapabilityState            *anchor,
		...
		public:
		[[nodiscard]] static constexpr bool is_sub_quota_id(uint16_t value)
		{
			return value >= IdMin;
		}
		 ...
		 * allocation policy. Returns true on success, false if the ID space is
		 * exhausted.
		 */
		bool allocate_id(PrivateAllocatorCapabilityState *node)
		{
			if (node == nullptr)
			{
				return false;
			...
			if (head.is_null())
			{
				auto     offset  = Policy::pick_offset(IdRange);
			...
			do
			{
				...
				if (gap > 0)
				{
					uint16_t offset = Policy::pick_offset(gap);
					Debug::Assert(offset < gap,
					              "Policy::pick_offset returned offset outside "
					              "gap [{}, {})",
					...
					// pointer
					insert_after(current, node, nodeOffset, id);
					head = nodeOffset;
					return true;
				}
		 ...
		 */
		void release_id(PrivateAllocatorCapabilityState *node)
		{
			if (node == nullptr)
			{
				return;
			...
			Debug::Assert(!head.is_null(), "release_id called on empty ring");
			Debug::Assert(!node->next.is_null() && !node->prev.is_null(),
	...
	class Claim
	{
		...
		/**
		 * Transfers ownership of this claim. Cannot transfer to ID 0.
		 */
		void transfer_ownership(uint16_t newOwner)
		{
			if (newOwner != 0)
			{
...
namespace
{
	 ...
	 */
	size_t inline calculate_allocation_size(size_t requestedSize)
	{
	                                      ...
	                                      PermissionSet       permissions)
	{
		...
		size_t sealedSize = calculate_allocation_size(requestedSize);
		if (sealedSize == 0)
		{
			return {nullptr, nullptr};
		}

		{
			...
			return allocate_sealed_unsealed_internal(
			  std::move(g), timeout, capability, key, sealedSize);
		}
	}
...
__cheriot_minimum_stack(0x270) AllocatorCapability
  split_sub_quota(Timeout *timeout, AllocatorCapability parent, size_t size)
{
	STACK_CHECK(0x270);
	if (!check_timeout_pointer(timeout))
	{
	...
	size_t sealedSize =
	  calculate_allocation_size(sizeof(PrivateAllocatorCapabilityState));

	if (parentCap->quota < (size + sealedSize))
	{
	...
	auto [sealedWrapped, unsealedWrapped] = allocate_sealed_unsealed_internal(
	  std::move(g), timeout, parentCap, key, sealedSize);
	...
	// Cursed? SealedTokenHandle doesn't expose .get() so we pun through
	// pointer for correct return value.
	...
	if (subQuotaIds.allocate_id(unsealed))
	{
		return sealed;
	}

	Debug::log<DebugLevel::Warning>(
	  "Failed to allocate ID in split_sub_quota.");
	unsealedWrapped.address() = unsealedWrapped.base();

	int ret = heap_free_internal(parent, unsealedWrapped, true);

	if (ret < 0)
	{
		Debug::log<DebugLevel::Warning>(
		  "Failed to free allocated child after ID allocation failure. "
		  "Free manually with recombine_sub_quota.");
		return reinterpret_cast<AllocatorCapability>(ret);
	}
...
// WHAT ABOUT CLAIMS ON QUOTAS? DO WE ALLOW?
__cheriot_minimum_stack(0x1a0) int recombine_sub_quota(
  ...
  AllocatorCapability parent)
{
				...
				else
				{
					...
					if (childClaim != nullptr)
					{
						...
						else if (chunk->ownerID == parentID)
						{
							...
							chunk->set_owner(0);
							childClaim->transfer_ownership(parentID);
	...
	subQuotaIds.release_id(childState);
